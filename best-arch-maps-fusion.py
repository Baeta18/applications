...
def generateFusion(groundPath,outputPath,instance,fusionInstances,data,crops):
	...
	for i in xrange(0,len(data),crops):
		...
		fusionFile =  resultPath + str(instance) + "_fusion_best_" + fusionInstances[imageCont] + ".png"
...
def main():
	...
	for i in fusionInstances:
		for cropSize in cropSizes: 
			...
			if(int(cropSize) == 33 or int(cropSize) == 41):
			...
			numProbs += 1
	generateFusion(groundPath,outputPath,instance,fusionInstances,probsData,(numProbs/len(fusionInstances)))
